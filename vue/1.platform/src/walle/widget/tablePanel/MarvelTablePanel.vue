<template>
  <div class="marvelTablePanelWrapper">
    ...
    <div v-if="divDirection == 'horizontal'" class="marvelTablePanel_horizontal">
      <div v-if="showFirst" class="firstDiv_horizontal" :style="{width:showSecond?'calc(' + firstDivW + ' - ' + (calW + dragSpace/2) +'px)':'100%'}">
        <slot name="firstDiv"></slot>
      </div>
      ...
      <div v-if="showSecond" class="second_horizontal" :style="{width:showFirst?'calc(' + secondDivW + ' + ' + (calW - dragSpace/2) +'px)':'100%'}">
        <slot name="secondDiv"></slot>
      </div>
    </div>
  </div>
